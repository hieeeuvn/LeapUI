...
#include <stdint.h>
#include <string.h>
// 8x8 bitmap ascii 32-127, 1 byte per row, 8 rows per char
static const uint8_t font8x8[96][8] = {
 ...
 {0x00,0x00,0x6C,0xFE,0xFE,0x66,0x66,0x00}, //109 m
...
void font_init(void){}
int font_measure(const char *t){ return t? (int)strlen(t)*8 : 0; }
int font_measure_text(const char *t){ return font_measure(t); }
void font_draw(uint16_t *fb,int sw,int sh,int x,int y,const char *t,uint16_t col){
    if(!fb||!t) return;
    int cx=x;
    for(const char *p=t;*p;p++){
        unsigned char c=(unsigned char)*p;
        if(c<32||c>127){ cx+=8; continue; }
        const uint8_t *g=font8x8[c-32];
        for(int row=0;row<8;row++){
            uint8_t bits=g[row];
            for(int bx=0;bx<8;bx++) if(bits & (0x80>>bx)){
                int px=cx+bx, py=y+row;
                if(px>=0&&px<sw&&py>=0&&py<sh) fb[py*sw+px]=col;
            }
        }
        cx+=8;
        if(cx>=sw) break;
...
void font_draw_text(uint16_t *fb,int w,int h,int x,int y,const char *t,uint16_t c){ font_draw(fb,w,h,x,y,t,c); }
void font_draw_char(uint16_t *fb,int w,int h,int x,int y,char ch,uint16_t c){ char s[2]={ch,0}; font_draw(fb,w,h,x,y,s,c); }
void font_load_from_settings(const char *n){ (void)n; }
